---
title: "09 — Cierre"
description: "Mini-chequeo, vocabulario y resumen de la unidad SCRUM Lite"
---

import StoryIntro from '@components/StoryIntro.astro';
import Aclaracion from '@components/Aclaracion.astro';
import Comparativa from '@components/Comparativa.astro';

> 🗺️ **Estás en:** 📋 **UD 1.2 · SCRUM Lite** → 09 · Cierre

<StoryIntro icono="🎉" titulo="Ya sabes qué es SCRUM. Ahora toca usarlo.">
  Has recorrido toda la unidad: desde qué es el enfoque ágil hasta cómo
  planificar sprints con plantillas. Ahora toca **verificar que lo entiendes**
  y **prepararte para la siguiente unidad**.
</StoryIntro>

## 📬 La idea en una frase

> **SCRUM Lite es trabajar en ciclos cortos con objetivos claros, reuniones rápidas y un tablero visible.**

Si recuerdas esto, has entendido la unidad.

---

## 🎯 Mini-chequeo final

Responde estas preguntas antes de seguir:

### Preguntas de comprensión

<details>
<summary>¿Cuál es la diferencia entre Waterfall y Agile?</summary>

**Respuesta**: Waterfall planifica todo al principio y entrega al final. Agile planifica poco, entrega en ciclos cortos y se adapta al cambio.

**Para qué sirve**: Para entender por qué SCRUM existe y qué problema resuelve.
</details>

<details>
<summary>¿Cuáles son los 3 roles de SCRUM?</summary>

**Respuesta**: Product Owner (prioriza el trabajo), Scrum Master (facilita el proceso) y Development Team (ejecuta). En tu proyecto, estos roles se simplifican o se rotan.

**Para qué sirve**: Para que cada persona sepa qué responsabilidad tiene.
</details>

<details>
<summary>¿Cuánto dura un sprint recomendado para tu proyecto?</summary>

**Respuesta**: **1 semana**. Con 6 semanas de proyecto, tienes 6 sprints. Es suficiente para entregar algo bien hecho y tener feedback frecuente.

**Para qué sirve**: Para planificar tu proyecto en ciclos manejables.
</details>

<details>
<summary>¿Qué es un tablero Kanban y por qué es importante?</summary>

**Respuesta**: Un tablero con columnas (To Do, In Progress, Done) que visualiza todo el trabajo del equipo. Es importante porque hace visible el progreso y los bloqueos.

**Para qué sirve**: Para que todos sepan qué falta, qué se está haciendo y qué está terminado.
</details>

<details>
<summary>¿Cuál es el formato de una user story?</summary>

**Respuesta**: **Como [usuario], quiero [función], para [beneficio]**. Ejemplo: "Como alumno, quiero crear tareas con fecha para organizarme."

**Para qué sirve**: Para pensar en el usuario y en el valor, no solo en el código.
</details>

<details>
<summary>¿Qué hacer si una feature es XL en estimación?</summary>

**Respuesta**: **Dividirla** en features más pequeñas. Una feature que cuesta más de 3 días probablemente es demasiado grande para un sprint.

**Para qué sirve**: Para que cada tarea sea manejable y terminable en una semana.
</details>

<details>
<summary>¿Cuánto tiempo se invierte en reuniones SCRUM por semana?</summary>

**Respuesta**: Aproximadamente **1 hora**: planning 30 min + daily 5×3 min + retrospectiva 15 min.

**Para qué sirve**: Para demostrar que SCRUM no es burocracia, es una inversión con gran retorno.
</details>

---

### Ejercicio práctico

**En 15 minutos, prepara tu primer sprint:**

1. Crea un tablero en Trello o GitHub Projects con las columnas To Do, In Progress, Done
2. Añade 3-5 user stories de tu proyecto como tarjetas en "To Do"
3. Estima cada story con T-shirt sizes (S, M, L)
4. Asigna responsables
5. Define el objetivo del sprint

**Si consigues tener un tablero listo con 3-5 tarjetas → has entendido la unidad.**

---

## 📊 Tabla resumen de la unidad

| Concepto | Idea clave |
|----------|------------|
| **Ágil** | Trabajar en ciclos cortos, entregar valor, adaptarse al cambio |
| **SCRUM** | Framework con roles, ceremonias y artefactos |
| **Sprint** | Ciclo de 1-2 semanas con objetivo y entrega |
| **Kanban** | Tablero con To Do → In Progress → Done |
| **User Stories** | "Como... quiero... para que..." con criterios de aceptación |
| **Estimación** | T-shirt sizes, Story Points, Planning Poker |
| **Errores** | Los 6 pecados: rechazar SCRUM, odiar reuniones, no usar tablero... |
| **Templates** | Planning, daily, retrospectiva, burndown, DoD |

---

## 📖 Vocabulario rápido

| Término | Definición |
|---------|-----------|
| **Ágil** | Enfoque de desarrollo en ciclos cortos con feedback continuo |
| **SCRUM** | Framework ágil con roles, ceremonias y artefactos definidos |
| **Sprint** | Período de tiempo (1-2 semanas) con objetivo y entrega |
| **Product Owner** | Rol que define qué se hace y prioriza el trabajo |
| **Scrum Master** | Rol que facilita el proceso y quita impedimentos |
| **Daily Scrum** | Reunión diaria de 15 minutos (¿qué hice? ¿qué haré? ¿qué bloquea?) |
| **Sprint Planning** | Reunión para decidir qué se hace en el sprint |
| **Sprint Review** | Reunión para mostrar lo que se ha hecho |
| **Retrospectiva** | Reunión para mejorar cómo se trabaja |
| **Tablero Kanban** | Herramienta visual con columnas To Do, In Progress, Done |
| **WIP** | Work In Progress — máximo de tareas simultáneas |
| **User Story** | Descripción de una funcionalidad desde el punto de vista del usuario |
| **Story Points** | Medida relativa de esfuerzo (escala Fibonacci: 1, 2, 3, 5, 8, 13) |
| **Planning Poker** | Técnica de estimación consensuada en equipo |
| **T-shirt Sizes** | Método de estimación con tallas: XS, S, M, L, XL |
| **Burndown Chart** | Gráfico que muestra el trabajo restante día a día |
| **Definition of Done** | Criterios que deben cumplirse para considerar una tarea terminada |
| **Backlog** | Lista de todas las features pendientes, ordenadas por prioridad |
| **Increment** | Resultado tangible del sprint: software funcionando |

---

## 🔗 Conexión con las siguientes unidades

| Unidad | Qué aprenderás | Cómo se conecta |
|--------|---------------|-----------------|
| **1.3 Requisitos** | Cómo documentar qué hace tu app | Ampliarás las user stories del sprint |
| **1.4 Elección de Tecnologías** | Cómo elegir tu stack | Elegirás el stack para el sprint |
| **1.5 Comunicación** | Cómo presentar tu proyecto | Mostrarás el resultado de tus sprints |
| **1.7 Propuesta** | Cómo escribir la propuesta final | El historial de sprints documenta tu progreso |

---

## 🚀 Cheat sheet: acciones rápidas con opencode

| Tarea | Prompt |
|-------|--------|
| **Planning rápido** | `Dame un planning de sprint para 3 personas con 5 features: [lista]. Estima en story points y asigna tareas` |
| **Daily structure** | `Crea un formato de daily de 15 min para un equipo de 3 personas trabajando en [tu proyecto]` |
| **Retrospectiva** | `Dame 3 cosas que salieron bien, 3 que salieron mal y 3 acciones concretas para el equipo [descripción]` |
| **Estimar features** | `Estima estas features en T-shirt sizes: [lista]. Somos [N] personas con experiencia en [tech]` |
| **DoD checklist** | `Crea una Definition of Done para un proyecto web con Vue + Tailwind + tests` |
| **Kanban board** | `Diseña un tablero Kanban con columnas para un sprint de [tu proyecto]` |

---

## 💡 Consejo para el siguiente punto

Si todavía no tienes claro cómo organizar tu proyecto:

1. Crea un tablero en Trello ahora mismo (5 minutos)
2. Añade las 5 features principales de tu proyecto
3. Estímalas con T-shirt sizes
4. Elige las 3 que entran en el primer sprint
5. Empieza el lunes con el planning

**No necesitas SCRUM perfecto. Necesitas empezar.**

---

## ✅ Resumen en 3 frases

1. SCRUM Lite es trabajar en **sprints de 1 semana** con planning, daily y retrospectiva: **1 hora por semana** de inversión.
2. Un **tablero Kanban** visualiza todo el trabajo y hace visible el progreso y los bloqueos.
3. Las **user stories** y la **estimación** te obligan a pensar en el usuario y en el esfuerzo antes de programar.

---

<div class="nav-unidad">
<div class="nav-unidad-links">
<a class="nav-box nav-box-prev" href="../08-template-sprint">
  <span class="nav-box-dir">← Anterior</span>
  <span class="nav-box-title">08 — Template Sprint</span>
</a><a class="nav-box nav-box-next" href="../../u1-3-requisitos">
  <span class="nav-box-dir">Siguiente →</span>
  <span class="nav-box-title">UD 1.3 — Requisitos</span>
</a>
</div>
<a class="nav-unidad-volver" href="../"> Volver al índice de la unidad</a>
</div>
